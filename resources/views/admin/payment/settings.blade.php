    ...
    <div class="container-fluid">
        <div class="box box-block bg-white">
            <form action="{{route('admin.settings.payment.store')}}" method="POST">
                ...
                <div class="card card-block card-inverse card-primary">
                    <blockquote class="card-blockquote">
                        <i class="fa fa-3x fab fa-cc-mastercard pull-right"></i>
                        <div class="form-group row">
                            <div class="col-xs-4">
                                <label for="paystack_secret_key" class="col-form-label">
                                    @lang('admin.payment.card_payments2')
                                </label>
                            </div>
                            <div class="col-xs-6">
                                <input @if(Setting::get('CARD2') == 1) checked  @endif  name="CARD2" id="paystack_check" onchange="cardselect2()" type="checkbox" class="js-switch" data-color="#43b968">
                            </div>
                        </div>
                        <div id="card_field2" @if(Setting::get('CARD2') == 0) style="display: none;" @endif>
                            <div class="form-group row">
                                <label for="paystack_secret_key" class="col-xs-4 col-form-label">@lang('admin.payment.paystack_secret_key')</label>
                                <div class="col-xs-8">
                                    <input class="form-control" type="text" value="{{Setting::get('paystack_secret_key', '') }}" name="paystack_secret_key" id="paystack_secret_key"  placeholder="Paystack Secret key">
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="paystack_publishable_key" class="col-xs-4 col-form-label">@lang('admin.payment.paystack_publishable_key')</label>
                                <div class="col-xs-8">
                                    <input class="form-control" type="text" value="{{Setting::get('paystack_publishable_key', '') }}" name="paystack_publishable_key" id="paystack_publishable_key"  placeholder="Paystack Publishable key">
                                </div>
                            </div>
                        </div>
                    </blockquote>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-xs-8">
                                <select name="currency" class="form-control" required>
                                    <option @if(Setting::get('currency') == "$") selected @endif value="₦">Nigerian Naira (NGN)</option>
...
<!-- Paystack checkbos script -->
<script type="text/javascript">
function cardselect2()
{
    if($('#paystack_check').is(":checked")) {
        $("#card_field2").fadeIn(700);
    } else {
        $("#card_field2").fadeOut(700);
    }
}
</script>
@endsection